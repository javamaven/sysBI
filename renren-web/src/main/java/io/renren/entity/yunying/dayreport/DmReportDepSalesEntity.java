package io.renren.entity.yunying.dayreport;

import java.io.Serializable;



/**
 * 
 * 
 * @author chenshun
 * @email sunlightcs@gmail.com
 * @date 2017-06-29 16:30:26
 */
public class DmReportDepSalesEntity implements Serializable {
	private static final long serialVersionUID = 1L;
	
	//日期
	private String statPeriod;
	//15月
	private String shiwutian;
	private String zichan;
	//子类
	private String salesType;
	//期限
	private String yiyue;
	//金额
	private String eryue;
	private String sanyue;
	private String siyue;
	private String wuyue;
	private String liuyue;
	private String bayue;
	private String jiuyue;
	private String shiyue;
	private String shiyiyue;
	private String shieryue;
	private String shiwuyue;
	private String shibayue;
	private String ershisiyue;
	private String sanshiwuyue;
	private String sanshiliuyue;
	private String sishibayue;
	private String liushiyue;
	private String jiushiliuyue;
	private String zongji;
	private String zhanbi;
	
	private String weekTongRate;//周同比
	
	
	
	
	public String getSiyue() {
		return siyue;
	}

	public void setSiyue(String siyue) {
		this.siyue = siyue;
	}

	public String getWuyue() {
		return wuyue;
	}

	public void setWuyue(String wuyue) {
		this.wuyue = wuyue;
	}

	public String getShiyiyue() {
		return shiyiyue;
	}

	public void setShiyiyue(String shiyiyue) {
		this.shiyiyue = shiyiyue;
	}

	public void setWeekTongRate(String weekTongRate) {
		this.weekTongRate = weekTongRate;
	}
	
	public String getWeekTongRate() {
		return weekTongRate;
	}

	public String getZichan() {
		return zichan;
	}
	public void setZichan(String zichan) {
		this.zichan = zichan;
	}
	public String getBayue() {
		return bayue;
	}
	public void setBayue(String bayue) {
		this.bayue = bayue;
	}
	public String getShiyue() {
		return shiyue;
	}
	public void setShiyue(String shiyue) {
		this.shiyue = shiyue;
	}
	public String getShiwuyue() {
		return shiwuyue;
	}
	public void setShiwuyue(String shiwuyue) {
		this.shiwuyue = shiwuyue;
	}
	public String getShibayue() {
		return shibayue;
	}
	public void setShibayue(String shibayue) {
		this.shibayue = shibayue;
	}
	public String getErshisiyue() {
		return ershisiyue;
	}
	public void setErshisiyue(String ershisiyue) {
		this.ershisiyue = ershisiyue;
	}
	public String getSanshiwuyue() {
		return sanshiwuyue;
	}
	public void setSanshiwuyue(String sanshiwuyue) {
		this.sanshiwuyue = sanshiwuyue;
	}
	public String getSanshiliuyue() {
		return sanshiliuyue;
	}
	public void setSanshiliuyue(String sanshiliuyue) {
		this.sanshiliuyue = sanshiliuyue;
	}
	public String getSishibayue() {
		return sishibayue;
	}
	public void setSishibayue(String sishibayue) {
		this.sishibayue = sishibayue;
	}
	public String getLiushiyue() {
		return liushiyue;
	}
	public void setLiushiyue(String liushiyue) {
		this.liushiyue = liushiyue;
	}
	public String getJiushiliuyue() {
		return jiushiliuyue;
	}
	public void setJiushiliuyue(String jiushiliuyue) {
		this.jiushiliuyue = jiushiliuyue;
	}
	public String getShiwutian() {
		return shiwutian;
	}
	public void setShiwutian(String shiwutian) {
		this.shiwutian = shiwutian;
	}
	public String getYiyue() {
		return yiyue;
	}
	public void setYiyue(String yiyue) {
		this.yiyue = yiyue;
	}
	public String getEryue() {
		return eryue;
	}
	public void setEryue(String eryue) {
		this.eryue = eryue;
	}
	public String getSanyue() {
		return sanyue;
	}
	public void setSanyue(String sanyue) {
		this.sanyue = sanyue;
	}
	public String getLiuyue() {
		return liuyue;
	}
	public void setLiuyue(String liuyue) {
		this.liuyue = liuyue;
	}
	public String getJiuyue() {
		return jiuyue;
	}
	public void setJiuyue(String jiuyue) {
		this.jiuyue = jiuyue;
	}
	public String getShieryue() {
		return shieryue;
	}
	public void setShieryue(String shieryue) {
		this.shieryue = shieryue;
	}
	public String getZongji() {
		return zongji;
	}
	public void setZongji(String zongji) {
		this.zongji = zongji;
	}
	public String getZhanbi() {
		return zhanbi;
	}
	public void setZhanbi(String zhanbi) {
		this.zhanbi = zhanbi;
	}
	public static long getSerialversionuid() {
		return serialVersionUID;
	}
	/**
	 * 设置：日期
	 */
	public void setStatPeriod(String statPeriod) {
		this.statPeriod = statPeriod;
	}
	/**
	 * 获取：日期
	 */
	public String getStatPeriod() {
		return statPeriod;
	}
	/**
	 * 设置：分类
	 */

	/**
	 * 设置：子类
	 */
	public void setSalesType(String salesType) {
		this.salesType = salesType;
	}
	/**
	 * 获取：子类
	 */
	public String getSalesType() {
		return salesType;
	}
	/**
	 * 设置：期限
	 */

	@Override
	public String toString() {
		return "DmReportDepSalesEntity [statPeriod=" + statPeriod + ", shiwutian=" + shiwutian + ", zichan=" + zichan
				+ ", salesType=" + salesType + ", yiyue=" + yiyue + ", eryue=" + eryue + ", sanyue=" + sanyue
				+ ", liuyue=" + liuyue + ", bayue=" + bayue + ", jiuyue=" + jiuyue + ", shiyue=" + shiyue
				+ ", shieryue=" + shieryue + ", shiwuyue=" + shiwuyue + ", shibayue=" + shibayue + ", ershisiyue="
				+ ershisiyue + ", sanshiwuyue=" + sanshiwuyue + ", sanshiliuyue=" + sanshiliuyue + ", sishibayue="
				+ sishibayue + ", liushiyue=" + liushiyue + ", jiushiliuyue=" + jiushiliuyue + ", zongji=" + zongji
				+ ", zhanbi=" + zhanbi + ", weekTongRate=" + weekTongRate + "]";
	}
	
}
