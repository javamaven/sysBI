...
public class VipPotData implements Job {
	public final Logger log = Logger.getLogger(this.getClass());
	private ScheduleReportTaskService taskService = SpringBeanFactory.getBean(ScheduleReportTaskService.class);
	DmReportPotVipService service = SpringBeanFactory.getBean(DmReportPotVipService.class);
	...
	private boolean run(JobExecutionContext ctx) {
		...
		try {
			...
			List<DmReportPotVipEntity> queryList = service.queryList(queryParams);
			JSONArray dataArray = new JSONArray();
			for (int i = 0; i < queryList.size(); i++) {
				DmReportPotVipEntity entity = queryList.get(i);
