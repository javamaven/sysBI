...
public class SqlConstants {
	...
	public static String currDay = "";//投资笔数
	public static String currDayAmount = "";//投资金额
...
//			"END " ;
	...
	static String cg_total_amount = "SELECT SUM(tender_capital) CG_TOTAL_AMOUNT FROM ( SELECT sum( IFNULL( CASE a.tender_subject WHEN 1 THEN a.tender_capital / 100 WHEN 2 THEN b.pay_amount / 100 END, 0 )) AS tender_capital FROM project_tender_detail a LEFT JOIN creditor_purchase_order b ON a.id = b.relate_tender_detail_id LEFT JOIN easy_aggregate_order_detail o ON (a.id = o.tender_detail_id) LEFT JOIN financial_plan_order_detail p ON (a.id = p.tender_detail_id) WHERE a.tender_account_status IN (0, 1) AND o.tender_detail_id IS NULL AND p.tender_detail_id IS NULL UNION ALL SELECT sum( IFNULL(tender_amount / 100, 0)) FROM financial_plan_order UNION ALL SELECT sum( IFNULL(tender_amount / 100, 0)) FROM easy_aggregate_order ) S";
//			"SELECT " +
...
//			"  ) S ";
	...
	/******交易趋势图***************************************************************************************************/
	//存管版交易金额
	public static String cg_invest_amount_list = "SELECT tender_capital MONEY, time TIME FROM ( SELECT IFNULL( CASE a.tender_subject WHEN 1 THEN a.tender_capital / 100 WHEN 2 THEN b.pay_amount / 100 END, 0 ) tender_capital, a.addtime TIME FROM project_tender_detail a LEFT JOIN creditor_purchase_order b ON a.id = b.relate_tender_detail_id LEFT JOIN financial_plan_order_detail f ON (a.id = f.tender_detail_id) LEFT JOIN easy_aggregate_order_detail e ON (a.id = e.tender_detail_id) WHERE a.tender_account_status IN (0, 1) AND f.tender_detail_id IS NULL AND e.tender_detail_id IS NULL AND a.addtime >= ? UNION ALL SELECT IFNULL(tender_amount / 100, 0), tender_time TIME FROM financial_plan_order WHERE 1 = 1 AND tender_time >= ? UNION ALL SELECT IFNULL(tender_amount / 100, 0), tender_time TIME FROM easy_aggregate_order WHERE 1 = 1 AND tender_time >= ? ) S";
	//普通版项目交易金额
	...
	//存管版交易金额
	public static String cg_30minutes_invest_amount = "SELECT SUM(tender_capital) CG_TOTAL_AMOUNT FROM ( SELECT IFNULL( CASE a.tender_subject WHEN 1 THEN a.tender_capital / 100 WHEN 2 THEN b.pay_amount / 100 END, 0 ) tender_capital FROM project_tender_detail a LEFT JOIN creditor_purchase_order b ON a.id = b.relate_tender_detail_id LEFT JOIN financial_plan_order_detail f ON (a.id = f.tender_detail_id) LEFT JOIN easy_aggregate_order_detail e ON (a.id = e.tender_detail_id) WHERE a.tender_account_status IN (0, 1) AND f.tender_detail_id IS NULL AND e.tender_detail_id IS NULL AND a.addtime >= ? AND a.addtime < ? UNION ALL SELECT IFNULL(tender_amount / 100, 0) FROM financial_plan_order WHERE 1 = 1 AND tender_time >= ? AND tender_time < ? UNION ALL SELECT IFNULL(tender_amount / 100, 0), tender_time TIME FROM easy_aggregate_order WHERE 1 = 1 AND tender_time >= ? AND tender_time < ? ) S";
	//普通版项目交易金额
	...
	public static String curr_cg_invest_sql = "SELECT round(sum(tender_capital), 0) MONEY FROM ( SELECT IFNULL( CASE a.tender_subject WHEN 1 THEN a.tender_capital / 100 WHEN 2 THEN b.pay_amount / 100 END, 0 ) tender_capital, a.addtime TIME FROM project_tender_detail a LEFT JOIN creditor_purchase_order b ON a.id = b.relate_tender_detail_id LEFT JOIN financial_plan_order_detail f ON (a.id = f.tender_detail_id) LEFT JOIN easy_aggregate_order_detail e ON (a.id = e.tender_detail_id) WHERE a.tender_account_status IN (0, 1) AND f.tender_detail_id IS NULL AND e.tender_detail_id IS NULL AND a.addtime >= ? AND a.addtime <= ? UNION ALL SELECT IFNULL(tender_amount / 100, 0), tender_time TIME FROM financial_plan_order WHERE 1 = 1 AND tender_time >= ? AND tender_time <= ? UNION ALL SELECT IFNULL(tender_amount / 100, 0), tender_time TIME FROM easy_aggregate_order WHERE 1 = 1 AND tender_time >= ? AND tender_time <= ? ) S";
	...
	public static String curr_invest_times_sql = 
					...
					" ) s ";
	//存管版投资 调用 sqlcg_invest_amount_list
	public static String curr_cg_invest_times_sql = "SELECT count(1) invest_times FROM ( SELECT IFNULL( CASE a.tender_subject WHEN 1 THEN a.tender_capital / 100 WHEN 2 THEN b.pay_amount / 100 END, 0 ) tender_capital, a.addtime TIME FROM project_tender_detail a LEFT JOIN creditor_purchase_order b ON a.id = b.relate_tender_detail_id LEFT JOIN financial_plan_order_detail f ON (a.id = f.tender_detail_id) LEFT JOIN easy_aggregate_order_detail e ON (a.id = e.tender_detail_id) WHERE 1 = 1 AND a.tender_account_status IN (0, 1) AND f.tender_detail_id IS NULL AND e.tender_detail_id IS NULL AND a.addtime >= ? AND a.addtime <= ? UNION ALL SELECT IFNULL(tender_amount / 100, 0), tender_time TIME FROM financial_plan_order WHERE 1 = 1 AND tender_time >= ? AND tender_time <= ? UNION ALL SELECT IFNULL(tender_amount / 100, 0), tender_time TIME FROM easy_aggregate_order WHERE 1 = 1 AND tender_time >= ? AND tender_time <= ? ) S";
	...
	public static String curr_cg_invest_total_times_sql = "SELECT sum(TIME) invest_times FROM ( SELECT count(a.addtime) TIME FROM project_tender_detail a LEFT JOIN creditor_purchase_order b ON a.id = b.relate_tender_detail_id LEFT JOIN financial_plan_order_detail f ON (a.id = f.tender_detail_id) LEFT JOIN easy_aggregate_order_detail e ON (a.id = e.tender_detail_id) WHERE 1 = 1 AND a.tender_account_status IN (0, 1) AND f.tender_detail_id IS NULL AND e.tender_detail_id IS NULL UNION ALL SELECT count(tender_time) TIME FROM financial_plan_order WHERE 1 = 1 UNION ALL SELECT count(tender_time) TIME FROM easy_aggregate_order WHERE 1 = 1 ) S";
